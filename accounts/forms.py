    ...
    def clean(self, *args, **kwargs):
        ...
        if email and password:
            ...
            if not check_password(password, qs[0].password):
                raise forms.ValidationError('Пароль не верен.')
...
class UserRegistrationForm(forms.ModelForm):
    email = forms.EmailField(label='введите электронную почту', widget=forms.EmailInput(attrs={'class': 'form-control'}))
...
class UserUpdateForm(forms.Form):
    ...
    send_email = forms.BooleanField(required=False, widget=forms.CheckboxInput, label='Получать разсылку?')
...
class ContactForm(forms.Form):
    ...
    email = forms.EmailField(required=True, widget=forms.EmailInput(attrs={'class': 'form-control'}), label='введите электронную почту')
